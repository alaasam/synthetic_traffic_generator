...
NUMBER_OF_SYNTHETIC_USERS = 1516901
...
class User(object):
    def __init__(self, uid, klass, initial_timestamp_date=date(2013, 8, 25)):
        self.uid = uid
        self.klass = klass
    ...
    def find_traffic_model_per_hour(self):
                ...
                for index, time in enumerate(mean_inter_arrival_times):
                    if index == 0:
                        ...
                        arrival_datetimes.append(
                            datetime(self.initial_timestamp_date.year, self.initial_timestamp_date.month,
                                     self.initial_timestamp_date.day, hour) + timedelta(seconds=time))
                    else:
                        arrival_times.append(arrival_times[index - 1] + time)
                        arrival_datetimes.append(
                            datetime(self.initial_timestamp_date.year, self.initial_timestamp_date.month,
                                     self.initial_timestamp_date.day, hour) + timedelta(
                                seconds=arrival_times[index - 1] + time))
    ...
    def requests(self):
        for hour in range(1, 24):
            for filesize, arrival_datetime in zip(self.request_file_sizes_per_hour[hour],
                                                  self.request_arrival_datetimes_per_hour[hour]):
    ...
    def write_traffic_to_file(self):
        ...
        with open(path.join(user_syntethic_trace_path, "%s.dat" % self.uid), 'w') as user_syntethic_trace_file:
            for filesize, arrival_datetime in self.requests():
                # Remove the microsend part from the datetime
                user_syntethic_trace_file.write("%s %s %s %s\n" % (
                arrival_datetime.strftime("%Y-%m-%d %H:%M:%S"), self.uid, filesize,
                self.klass))  # 2013-08-25 00:10:58 13 30.7411159743, HF

    def generate_and_write_synthetic_traffic(self):
    ...
    def users(self):
        for uid in range(self.number_of_users):
            ...
            if probability <= self.mo_probability:
                ...
                klass = 'LF'
            elif probability <= self.mo_probability + self.lo_probability + self.ho_probability + self.lf_probability + self.mf_probability:
                klass = 'MF'
            else:
                klass = 'HF'
